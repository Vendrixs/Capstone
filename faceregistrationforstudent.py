...
def get_time_values():
    ...
    if not all(char.isalpha() or char in (',', '-') for char in ''.join(weekdays)):
        messagebox.showerror("Invalid Weekday", "Weekday should only contain alphabetical"
                                                " characters, commas, or dashes.")
        return
    ...
    if not all(weekdays):
        ...
        return
    if len(time_in_value) != len(time_out_value) or \
            len(time_in_value) != len(weekdays) or len(time_in_value) != len(subjects):
    ...
    if not course.isalpha():
        messagebox.showerror("No Course has been Entered", "Course should only contain alphabetical characters.")
        return

    section = section_entry.get()
    if not section:
        messagebox.showerror("No Year and Section has been Entered", "Please enter Year and Section")
        return

    schoolyear = schoolyear_combobox.get()
    if not schoolyear:
        messagebox.showerror("No SchoolYear has been Entered", "Please enter the SchoolYear")
        return
    ...
    confirmation_message = f"Notice! \nAll Inputs should correspond to each other \n " \
                           ...
                           f"Name: {Name_entry.get()}\nCourse: {course}\nYear&Section: {section_entry.get()}\n" \
                           f"SchoolYear: {schoolyear_combobox.get()}\nSubjects: {', '.join(subjects)}\n" \
...
def capture_face():
    ...
    subjects = [s.strip() for s in subject_str.split(',')]
    weekdays = [w.strip() for w in weekday_str.split(',')]
    # Fetch values for Course, Section, and SchoolYear
    course = course_combobox.get()
    section = section_entry.get()
    schoolyear = schoolyear_combobox.get()
...
root = tk.Tk()
root.title("Student Registration Window")
...
# Course input field
course_label = tk.Label(input_frame, text="Course:", bg="green", fg="white", font=("Arial", 14))
course_label.pack()

# Create a combo box for selecting the course
course_combobox = ttk.Combobox(input_frame, values=["BSIT", "BSGE", "BSABE"], state="readonly")
course_combobox.pack()

# Section input field
section_label = tk.Label(input_frame, text="Year & Section:", bg="green", fg="white", font=("Arial", 14))
section_label.pack()

section_entry = tk.Entry(input_frame, bg="white")
section_entry.pack()

# School Year input field
schoolyear_label = tk.Label(input_frame, text="School Year:", bg="green", fg="white", font=("Arial", 14))
schoolyear_label.pack()

# Create a combo box for selecting the school year
schoolyear_combobox = ttk.Combobox(input_frame, values=["2023-2024", "2024-2025", "2025-2026","2026-2027"], state="readonly")
schoolyear_combobox.pack()


def validate_schoolyear(char):
    if char.isdigit() or char == "-":
        return True
    return False


# Add validation to accept only numerical values and dash
schoolyear_combobox.config(validate="key", validatecommand=(root.register(validate_schoolyear), "%S"))
